...
export default function Home() {
  const { address, balance, hasCard, unreadCount, walletType, setExternalWallet, refresh } = useWallet();
  ...
  const copyAddress = async () => {
    if (address) { await Clipboard.setStringAsync(address); Alert.alert('✅ Copied!', 'Wallet address copied'); }
  };
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          ...
          {/* 👤 Profile icon — opens connect wallet if no wallet, else goes to profile */}
          <TouchableOpacity
            style={[styles.iconBtn, walletType === 'external' && styles.iconBtnConnected]}
            onPress={() => router.push('/profile')}
            onLongPress={() => setShowConnect(true)}
          >
            <Text style={styles.iconBtnEmoji}>{walletType === 'external' ? '🦊' : '👤'}</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      {walletType === 'external' && (
        <View style={styles.connectedBanner}>
          <Text style={styles.connectedText}>🦊 MetaMask Connected</Text>
          <TouchableOpacity onPress={() => setShowConnect(true)}>
            <Text style={styles.connectedSwitch}>Switch</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={[styles.card, hasCard ? styles.cardActive : styles.cardInactive]} onPress={() => router.push('/card')}>
          {hasCard ? (
            <>
              <View style={styles.cardTop}><Text style={styles.cardChip}>▪▪▪▪</Text><Text style={styles.cardNet}>Arc ⚡</Text></View>
              <Text style={styles.cardNumber}>•••• •••• •••• {address?.slice(-4)}</Text>
              <View style={styles.cardBottom}><Text style={styles.cardHolder}>NIMBUS CARD</Text><Text style={styles.cardBadge}>✓ ACTIVE NFT</Text></View>
            </>
          ) : (
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={styles.promoCard} onPress={() => router.push('/bazarc')}>
          ...
          <View style={styles.promoText}>
            ...
            <Text style={styles.promoSub}>Decentralized marketplace on Arc</Text>
          </View>
      ...
      <Modal visible={showReceive} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.addrBox}><Text style={styles.addrText} selectable>{address}</Text></View>
...
const styles = StyleSheet.create({
  ...
  iconBtnConnected: { borderColor: COLORS.warning, backgroundColor: 'rgba(255,179,71,0.1)' },
  ...
  connectedBanner: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'rgba(255,179,71,0.1)', borderRadius: 12, padding: 12, borderWidth: 1, borderColor: 'rgba(255,179,71,0.3)' },
  connectedText: { color: COLORS.warning, fontSize: 13, fontWeight: '600' },
  connectedSwitch: { color: COLORS.accent, fontSize: 13, fontWeight: '700' },
  ...
  cardNet: { color: 'rgba(255,255,255,0.9)', fontSize: 13, fontWeight: '600' },
  ...
  addrBox: { backgroundColor: COLORS.bg, borderRadius: 12, padding: 14, borderWidth: 1, borderColor: COLORS.cardBorder },
  addrText: { color: COLORS.textSecondary, fontSize: 12, textAlign: 'center' },
